...
export function CommandCreationDrawer() {
    ...
    useEffect(() => {
        ...
        const total = selectedServices.reduce((prev, curr) => prev + curr.service.currentVersion.price * curr.quantity, 0)
        setBillingPrice(total)
    }, [selectedServices])


    const {createCommand, creatingCommand} = useCreateCommand()
